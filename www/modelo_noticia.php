...
function obtenerImagenesPorNoticia($mysqli, $noticia_id) {
...
function obtenerComentariosPorNoticia($mysqli, $noticia_id) {
    $stmt = $mysqli->prepare(
...
function obtenerLugares($mysqli) {
...
function obtenerHashtagsPorNoticia($mysqli, $noticia_id) {
    $stmt = $mysqli->prepare(
...
function insertarComentario($mysqli, $noticia_id, $nombre, $email, $texto) {
    // Filtro de localidades: resalta pueblos en mayúsculas
    $lugares = obtenerLugares($mysqli);
